...
function CardOportunidades({ oportunidade, onOportunidadeAtualizada }: CardOportunidadesProps) {
    ...
    const getStatusColor = (status: string) => {
        ...
        if (statusLower.includes('prospecção')) return 'bg-blue-100 text-blue-800 border-blue-200';
        if (statusLower.includes('qualificação')) return 'bg-yellow-100 text-yellow-800 border-yellow-200';
        if (statusLower.includes('proposta')) return 'bg-purple-100 text-purple-800 border-purple-200';
        if (statusLower.includes('negociação')) return 'bg-orange-100 text-orange-800 border-orange-200';
        if (statusLower.includes('fechamento')) return 'bg-emerald-100 text-emerald-800 border-emerald-200';
        if (statusLower.includes('perdida')) return 'bg-red-100 text-red-800 border-red-200';
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200 hover:shadow-xl transition-all duration-300 overflow-hidden group">
                ...
                <div className="p-6 pb-4">
                    <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="ml-4">
                            <div className="w-12 h-12 bg-gradient-to-b from-[#167cf1] to-[#005de3] rounded-xl flex items-center justify-center shadow-lg">
                    ...
                    <div className="mb-4">
                        ...
                        <div className="text-sm text-slate-500">Valor da oportunidade</div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <div className="text-xs text-slate-500 uppercase tracking-wide font-medium mb-1">Abertura</div>
                            <div className="text-sm font-semibold text-slate-700">
                                {formatarData(oportunidade.data_abertura)}
                            </div>
                        </div>
                        <div>
                            <div className="text-xs text-slate-500 uppercase tracking-wide font-medium mb-1">Fechamento</div>
                            <div className="text-sm font-semibold text-slate-700">
                        ...
                        <div className="mb-4">
                            {diasRestantes > 0 ? (
                                <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                                    diasRestantes <= 7 ? 'bg-red-100 text-red-800' : 
                                ...
                                <div className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800">
                                    <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                                    </svg>
